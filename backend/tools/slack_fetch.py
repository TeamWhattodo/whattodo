    ...
    if uid:
        with get_session() as db:
            token_obj = db.execute(
                select(IntegrationCredentialORM).where(IntegrationCredentialORM.user_id == uid, IntegrationCredentialORM.source == "slack")
            ).scalar_one_or_none()
            ...
            if token_obj and token_obj.credentials_data:
                try:
                    ...
                    try:
                        ...
                        token_val = creds.get("bot_token", decrypted)
                    except json.JSONDecodeError:
                        token_val = decrypted
                    return WebClient(token=token_val)
                except Exception as e:
                    import logging
                    logging.error(f"Slack DB 토큰 로드 실패: {e}")

    raise RuntimeError("Slack 계정 연동이 필요합니다.")
...
@tool
def slack_list_channels(config: RunnableConfig, limit: int = 100) -> str:
    """봇이 참여 중인 Slack 채널 목록을 조회합니다. 반환된 채널은 모두 메시지 조회 가능."""
    try:
        result = _client(config).conversations_list(limit=limit, types="public_channel")
        data = result.data
...
@tool
def slack_get_channel_history(channel_id: str, config: RunnableConfig, limit: int = 50) -> str:
    ...
    return _safe(lambda: _client(config).conversations_history(channel=channel_id, limit=limit))


@tool
def slack_get_thread_replies(channel_id: str, thread_ts: str, config: RunnableConfig, limit: int = 50) -> str:
    """Slack 스레드 답글을 조회합니다."""
    return _safe(lambda: _client(config).conversations_replies(channel=channel_id, ts=thread_ts, limit=limit))


@tool
def slack_post_message(channel_id: str, text: str, config: RunnableConfig, thread_ts: str = "") -> str:
    ...
    try:
        result = _client(config).chat_postMessage(**kwargs)
        data = result.data
        sent_ts = data.get("ts", "")
...
@tool
def slack_search_messages(query: str, config: RunnableConfig, count: int = 20) -> str:
    """Slack 전체 메시지를 키워드로 검색합니다."""
    return _safe(lambda: _client(config).search_messages(query=query, count=count))


@tool
def slack_delete_message(channel_id: str, ts: str, config: RunnableConfig) -> str:
    """Slack 메시지를 삭제합니다. channel_id: C로 시작하는 채널 ID. ts: 메시지 타임스탬프. 반드시 사용자 확인 후 실행."""
    return _safe(lambda: _client(config).chat_delete(channel=channel_id, ts=ts))


@tool
def fetch_slack_as_items(channel_id: str, config: RunnableConfig, limit: int = 30) -> str:
    ...
    try:
        result = _client(config).conversations_history(channel=channel_id, limit=limit)
...
@tool
def fetch_slack_all_items(config: RunnableConfig, limit_per_channel: int = 30) -> str:
