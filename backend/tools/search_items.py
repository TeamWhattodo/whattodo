...
def search_past_items(user_id: int, query: str = "", date_range: str = "7d", status: str = None, source: str = None) -> list[dict]:
    """
    ...
    return _search(user_id=user_id, query=query, status=status, source=source)


def get_item_thread(user_id: int, item_id: str, source: str) -> list[dict]:
    """
    특정 항목의 스레드 전체를 조회한다.
    ...
    if source == "gmail":
        return _fetch_gmail_thread(user_id, item, source_id)
    if source == "slack":
        return _fetch_slack_thread(user_id, item, source_id)

    # calendar / jira / notion — 저장된 item 반환
    return [{"item_id": item_id, "source": source, "content": item.get("raw_content", ""), "item": item}]


def _fetch_gmail_thread(user_id: int, item: dict, thread_id: str | None) -> list[dict]:
    if not thread_id:
...
def _fetch_slack_thread(user_id: int, item: dict, source_id: str | None) -> list[dict]:
    ...
    try:
        ...
        result = _client(user_id=user_id).conversations_replies(channel=channel_id, ts=thread_ts, limit=50)
...
if __name__ == "__main__":
    print(search_past_items(1, status="pending"))
